...
export function defaultCropQuad(width: number, height: number, insetRatio = 0.035): DocumentQuad {
...
export function detectDocumentPage(data: Uint8ClampedArray, width: number, height: number): DocumentDetectionResult {
  ...
  const areaRatio = polygonArea(candidate) / total;
  ...
  const confidence = clamp(support * 0.55 + contrastConfidence * 0.25 + clamp(areaRatio / 0.5, 0, 1) * 0.2, 0, 1);
  const validGeometry = support > 0.52 && areaRatio > 0.16 && areaRatio < 0.94;
  ...
  return {
    quad,
    confidence,
    canAutoCapture: Boolean(quad) && confidence >= 0.6 && warnings.length === 0,
    warnings,
    frameWidth: width,
    frameHeight: height,
    focusScore,
